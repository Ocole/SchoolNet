...
/**
 * Created by Coleman on 2016/10/26.
 */
...
public class WifiAdmin {
     ...
     * 得到扫描结果 获取无线信号列表
     */
    public ScanResult mReturnScanResult() {
        // 每次点击扫描之前清空上一次的扫描结果
        ScanResult result = (ScanResult) mWifiManager.getScanResults();
     ...
     * 得到扫描结果 获取无线信号列表
     */
    public  List<String>  getMacScanResult() {
        // 每次点击扫描之前清空上一次的扫描结果
        ...
        if (listResult != null) {
            for (int i = 0; i < listResult.size(); i++) {
                mScanResult = listResult.get(i);
                listMac.add(mScanResult.BSSID.toUpperCase());
            }
            return listMac;
        }
        return null;
